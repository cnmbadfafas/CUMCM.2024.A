...
cita_chu(1,1)=57.0320766510155;%初始极角
v=zeros(224,1);%速度群
b=0.55./(2.*pi);%螺距
a=0.55*16-32*pi*b;%
r=zeros(224,1);%极径群
r(1,1)=4.99231530259274;%极径，龙头
...
dia=0.055;%孔的直径
d=0.275;%孔中心到板头的距离
x=zeros(224,1);%各把手初始位置的横坐标
y=zeros(224,1);%各把手初始位置的纵坐标
x(1,1)=4.420273946;%龙头初始位置据中心距离
y(1,1)=2.320428909;
%%初始位置
mytheta=@(t,theta)-1./(b*sqrt(1+theta.^2));
theta0=57.0320766510155;%从300s开始
dt=0.2; % 时间步长
flag=0;%触碰标志位
time=0;%300s后碰撞所花的时间
syms k
...
[k_result] = vpasolve(eqn,k,[57.0320766510155 57.0320766510155+2*pi]);
cita_chu(2,1)=double(k_result);
r(2,1)=b*cita_chu(2,1);
x(2,1)=r(2,1)*cos(cita_chu(2,1));
y(2,1)=r(2,1)*sin(cita_chu(2,1));
sum_low=57.0320766510155+cita_chu(2,1)-cita_chu(1,1);
sum_high=57.0320766510155+2*pi+cita_chu(2,1)-cita_chu(1,1);
...
while flag==0
    ...
    [tt,theta]=ode45(mytheta,tspan,cita_chu(1,1)); % 龙格库塔法求解
    cita_chu(1,1)=theta(3);
    r(1,1)=b*cita_chu(1,1);
    x(1,1)=r(1,1)*cos(cita_chu(1,1));
